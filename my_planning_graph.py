...
class ActionLayer(BaseActionLayer):
	...
	def _inconsistent_effects(self, actionA, actionB):
		...
		for actionA_effect in actionA.effects:
			for actionB_effect in actionB.effects:
				if (str(actionA_effect) == "~" + str(actionB_effect)) or (str(actionB_effect) == "~" + str(actionA_effect)):
					return True
		return False
	...
	def _interference(self, actionA, actionB):
		...
		for actionA_effect in actionA.effects:
			for actionB_precondition in actionB.preconditions:
				if (str(actionA_effect) == "~" + str(actionB_precondition)) or (str(actionB_precondition) == "~" + str(actionA_effect)):
					return True

		for actionB_effect in actionB.effects:
			for actionA_precondition in actionA.preconditions:
				if (str(actionB_effect) == "~" + str(actionA_precondition)) or (str(actionA_precondition) == "~" + str(actionB_effect)):
					return True
		return False
...
class LiteralLayer(BaseLiteralLayer):
	def _inconsistent_support(self, literalA, literalB):
		...
		layers.BaseLayer.parent_layer
		"""
		literalA_preactions_list = []
		literalB_preactions_list = []

		for action in self.parent_layer:
			if literalA in action.effects:
				literalA_preactions_list.append(action)
			if literalB in action.effects:
				literalB_preactions_list.append(action)

		return all(self.parent_layer.is_mutex(actionA, actionB) for actionB in literalB_preactions_list for actionA in literalA_preactions_list)
...
class PlanningGraph:
	def __init__(self, problem, state, serialize=True, ignore_mutexes=False):
		...
		self.level_costs_list = []
		self.current_level = 0
		level_sum = 0
		goal_literals = self.goal.copy()

		while goal_literals:
			literal_layer = self.literal_layers[-1]
			for literal in literal_layer:
				if literal in goal_literals:
					level_sum += self.current_level
					self.level_costs_list.append(self.current_level)
					goal_literals.remove(literal)
			self._extend()
	...
	def h_levelsum(self):
		...
		"""
		# TODO: implement this function
		return sum(self.level_costs_list)
	...
	def h_maxlevel(self):
		...
		return max(self.level_costs_list)
	...
	def h_setlevel(self):
		...
		for literal_layer in self.literal_layers:
			...
			for goal_literal in self.goal:
				if goal_literal not in literal_layer:
					all_goals_met = False
			...
			for goal_literalA in self.goal:
				for goal_literalB in self.goal:
					if literal_layer.is_mutex(goal_literalA, goal_literalB):
		...
		literal_layer = self.literal_layers[self.current_level]

		while True:
			if all(goal_literal in literal_layer for goal_literal in self.goal) and not any(self.literal_layers[current_level].is_mutex(goal_literal1, goal_literal2) for goal_literal1 in self.goal for goal_literal2 in self.goal):
				return current_level
			else:
				self._extend()
				literal_layer = self.literal_layers[-1]
